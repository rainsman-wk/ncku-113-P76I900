...
namespace SearchEnginesApp.Utils
{
    public partial class TfIdfAnalyzerForm : Form
    {
        ...
        private void InitializeForm()
        {
            ...
            {
                new DataGridViewTextBoxColumn { Name = "Rank", HeaderText = "#", Width = 60, AutoSizeMode = DataGridViewAutoSizeColumnMode.None },
        ...
        private void ShowDocumentKeySentences(string docId)
        {
            try
            {
                ...
                // Get results using Classic TF-IDF
                analyzer.SetTfIdfStrategy(false);
                var classicResults = analyzer.GetDocumentKeySentences(docId)
                    .Select((s, i) => new { Result = s, ClassicRank = i + 1 });

                // Get results using BM25
                analyzer.SetTfIdfStrategy(true);
                var bm25Results = analyzer.GetDocumentKeySentences(docId)
                    .Select((s, i) => new { Result = s, BM25Rank = i + 1 });
                ...
                UpdateStatus($"Analyzed document {docId} with both algorithms");
            }
            catch (Exception ex)
            {
